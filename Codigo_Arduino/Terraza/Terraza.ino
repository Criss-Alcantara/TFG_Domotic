...
void setup() { 
  ...
  //SENSOR LLUVIA
  pinMode (SENSOR_LLUVIA, INPUT); // D5
  // SENSOR MOVIMIENTO
...
void loop() {  
  ...
  if (currentMillis - previousMillis >= 5000) { //envia la temperatura cada 5 segundos
    ...
    lcd.print(HUMEDAD);
    lcd.print("%"); 
    Firebase.setInt(firebaseData,"house_1/Terraza/Temperatura",TEMPERATURA);
    Firebase.setInt(firebaseData,"house_1/Terraza/Humedad",HUMEDAD);
    previousMillis = currentMillis;
  }
  
//--------------------------------------------------------------
   //ESTADO LUZ 
  if(Firebase.getBool(firebaseData,"house_1/Terraza/Luz_Princ")){
    if(firebaseData.dataType() == "boolean"){
      if(firebaseData.boolData () == 1 && firebaseData.boolData () == aux_fb && digitalRead(ACTUADOR_PARED) == digitalRead(ACTUADOR_LUZ_ON_OFF_PRINC)){
       Firebase.setBool(firebaseData,"house_1/Terraza/Luz_Princ", false);
       aux_fb = 0; 
      }
       ...
       Firebase.setBool(firebaseData,"house_1/Terraza/Luz_Princ", true);
       aux_fb = 1; 
      }
...
//--------------------------------------------------------------
   //SENSOR LLUVIA
   if(Firebase.getBool(firebaseData,"house_1/Alarma/Sensores/S_Lluvia")){
    if(firebaseData.dataType() == "boolean"){
      if(digitalRead(SENSOR_LLUVIA) == 1){
        Firebase.setBool(firebaseData,"house_1/Alarma/Sensores/S_Lluvia", true);
      }
      else{
        Firebase.setBool(firebaseData,"house_1/Alarma/Sensores/S_Lluvia", false);
      }
    }
  }
  else {
